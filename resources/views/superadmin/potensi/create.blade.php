...
@section('title', 'Tambah Potensi Pasar')

@section('content')
<div class="p-6">
    <!-- Page Header -->
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 mb-2">Tambah Potensi Pasar</h1>
        <p class="text-gray-600">Tambahkan data potensi pasar baru</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        <div class="p-6">
            <form method="POST" action="{{ route('superadmin.potensi.store') }}">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="sumber_dana" id="sumber_dana"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            value="{{ old('sumber_dana') }}" placeholder="Contoh: APBD" required>
                        @error('sumber_dana')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="besar_anggaran" id="besar_anggaran" min="0"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            value="{{ old('besar_anggaran') }}" placeholder="Masukkan nilai anggaran" required>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="infrastruktur" id="infrastruktur" rows="4"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        placeholder="Masukkan deskripsi infrastruktur (opsional)">{{ old('infrastruktur') }}</textarea>
                    @error('infrastruktur')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="layanan" id="layanan" rows="4"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        placeholder="Masukkan deskripsi layanan (opsional)">{{ old('layanan') }}</textarea>
                    @error('layanan')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
